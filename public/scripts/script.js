...
function toggleSFW () {
  ...
  if (value === '1') {
    ...
    sfw.className = sfw.className.replace(' w3-border-green w3-text-green w3-hover-green', ' w3-border-red w3-text-red w3-hover-red')
  }
}
...
function fetchMonika () {
  ...
  if (document.getElementById('srcbtn').value === 'api') {
    fetch(api, { cache: 'no-cache', mode: 'cors' })
      .then(res => res.json())
...
function updateReddit (data) {
  ...
  if (!url.includes('https://i.redd.it/')) {
    ...
    if (isnsfw && document.getElementById('sfwbtn').value === 0) {
      fetchMonika()
    } else {
