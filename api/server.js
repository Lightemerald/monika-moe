...
app.get(base + '/get', (req, res) => {
    ...
    pool.query('SELECT * FROM images ORDER BY RAND() LIMIT 1', function(err, rows) {
...
app.post(base + '/add', (req, res) => {
    ...
    if (authorization === token) {
        if (data.status !== 'undefined' && data.filename) {
            pool.query(`INSERT INTO images(filename, nsfw) VALUES ("${data.filename.replace(/'/g, '\'')}", ${data.status})`, function(err) {
                if (!err) {
                    res.status(200).json({ code: 200, message: 'Image added!' });
                }
                else {
                    res.status(200).json({ code: 500, message: 'Failed to add image!', err: err });
                }
            });
        }
        else {
        ...
        if (data.id) {
            pool.query(`DELETE FROM images WHERE id=${data.id}`, function(err) {
                if (!err) {
                    ...
                    res.status(500).json({ code: 500, message: 'Failed to delete image!', err: err });
                }
            });
        }
        else {
    ...
    if (authorization === token) {
        if (data.id && data.filename) {
            ...
            if (data.status !== 'undefined') {
                query = `UPDATE images SET filename=\"${data.filename.replace(/'/g, '\'')}\" AND nsfw=${data.status} WHERE id=${data.id}`;
            }
            pool.query(query, function(err) {
                if (err) {
                    res.status(500).json({ code: 500, message: 'Failed to update AD!', err: err });
                }
